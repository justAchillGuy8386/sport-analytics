...
Được xây dựng bằng **Next.js 16 (App Router)**, **TypeScript**, **Tailwind CSS**, **Supabase (PostgreSQL)** và **GitHub Actions**.
Được xây dựng bằng **Next.js 16 (App Router)**, **TypeScript**, **Tailwind CSS**, **Supabase (PostgreSQL)**, **Goal API (goal-api.com)** và **GitHub Actions**.
...
* **Người dùng xem web tiêu tốn 0 API Quota**: Trình duyệt của người dùng chỉ truy vấn trực tiếp từ **Supabase Database**. Dù có hàng nghìn lượt truy cập cùng lúc, số lượt gọi API-Football vẫn là **0**.
...
* **Hạn ngạch hào phóng từ Goal API**: Gói Free cung cấp **1.000 requests/ngày** (gấp 10 lần so với API-Football cũ), hỗ trợ theo dõi đầy đủ 6 giải đấu và các trận đấu trực tiếp (LIVE).
* **Đồng bộ tự động thông minh (GitHub Actions CI/CD)**:
  * **Khung giờ cao điểm đêm (18:00 – 06:00 UTC+7)**: Tự động chạy **10 phút/lần** để cập nhật tỉ số, thẻ phạt, phạt góc và trận đấu đang diễn ra (**LIVE**).
  * **Khung giờ ban ngày (06:00 – 18:00 UTC+7)**: Tự động chạy **2 tiếng/lần** để đồng bộ lịch thi đấu và trạng thái mới nhất.
  * **Bắt trận LIVE tức thì**: Pipeline truy vấn trực tiếp endpoint `fixtures?live=all` trước tiên để ghi nhận diễn biến trực tiếp.
  * **Bắt trận LIVE tức thì**: Pipeline truy vấn trực tiếp endpoint `/fixtures/live` trước tiên để ghi nhận diễn biến trực tiếp.
* **Auto Polling phía Frontend**: Giao diện người dùng tự động cập nhật dữ liệu mới nhất từ Supabase mỗi 10 phút một lần hoàn toàn tự động.
...
* **Chế độ xem riêng từng giải**:
  * Khi chọn một giải (ví dụ: **Premier League**, **Champions League**, **La Liga**...):
...
* **Huy hiệu số trận trực quan trên Sidebar**: Hiển thị số lượng trận thực tế có trong database ngay cạnh tên giải.
...
| **BXH & Giải Đấu (Competition)** | Bảng xếp hạng chi tiết (vị trí, trận đã đá, hiệu số, điểm số) của 6 giải đấu lớn, tự động đồng bộ theo bộ lọc giải ở Sidebar. |
...
| **ETL & Quota Monitor** | Bảng giám sát hạn mức API-Football hàng ngày, nút kích hoạt nạp dữ liệu thủ công vào Supabase tức thì. |
...
* **Backend & API**: Next.js API Routes (`/api/football`, `/api/admin/sync`, `/api/football/standings`, `/api/football/quota`).
* **Cơ sở dữ liệu (Database)**: Supabase (PostgreSQL) với bảng `matches` được thiết lập khóa chính và cơ chế `upsert` idempotent.
* **Tự động hóa (Automation)**: GitHub Actions Scheduled Workflows (`.github/workflows/sync-supabase.yml`).
* **Khai thác dữ liệu (Data Source)**: API-Football v3 ([api-sports.io](https://api-sports.io/)).
...
```text
                          ...
                          +-------------------------------+
                                          |
                      (10 phút/lần đêm  |  2 tiếng/lần ngày)
                                          |
                                          v
                                           |
                       (10 phút/lần đêm  |  2 tiếng/lần ngày)
...
* **npm** hoặc **yarn** / **pnpm**.
* Tài khoản [Supabase](https://supabase.com/) và tài khoản [API-Football](https://dashboard.api-football.com/).
...
Cập nhật các giá trị cấu hình vào file `.env.local` (tất cả các khóa được bảo mật):
...
# 3. Kiểm tra biên dịch sản phẩm (Production Build)
...
## ⚙️ Cấu Hình GitHub Actions Tự Động (CI/CD)

Để pipeline tự động đồng bộ ngầm trên GitHub Actions, bạn cấu hình 3 biến **Repository Secrets** tại:
Để pipeline tự động đồng bộ ngầm trên GitHub Actions, bạn cấu hình các biến **Repository Secrets** tại:
`Settings -> Secrets and variables -> Actions -> New repository secret`

| Tên Secret | Mô Tả |
| :--- | :--- |
| `API_FOOTBALL_KEY` | Mã API Key của API-Football (v3) |
| `GOAL_API_KEY` | Mã API Key của Goal API (`gapi_...`) |
| `NEXT_PUBLIC_SUPABASE_URL` | URL dự án Supabase (`https://[project-id].supabase.co`) |
| `SUPABASE_SERVICE_ROLE_KEY` | Secret Key Service Role của Supabase (dùng quyền ghi dữ liệu) |

Workflow file được đặt tại: [`.github/workflows/sync-supabase.yml`](.github/workflows/sync-supabase.yml)
